  ...
  return (
    <div className="fade-in space-y-3">
      ...
      <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))' }}>
        <KpiCard label="sessions" value={formatNumber(d.totalChats)} sub={sel ? editorLabel(sel.id) : `${allEditors.length} editors`} onClick={() => navigate(selectedEditor ? `/sessions?editor=${selectedEditor}` : '/sessions')} infoTooltip="Total number of AI coding sessions recorded" />
        <KpiCard label="projects" value={d.topProjects.length} sub={`${daysSpan}d span`} onClick={() => navigate('/projects')} infoTooltip="Number of unique projects detected from your coding activity" />
        <KpiCard label="this month" value={thisMonth ? thisMonth.count : 0} sub={thisMonth ? thisMonth.month : ''} onClick={() => navigate('/sessions')} infoTooltip="Sessions recorded in the current month" />
        {stats && <>
          <KpiCard label="avg depth" value={avgMsgsPerSession} sub={<span className="flex items-center gap-0.5"><MessageSquare size={8} /> msgs/session</span>} infoTooltip="Average number of messages per AI session (conversation depth)" />
          <KpiCard label="tool calls" value={formatNumber(stats.totalToolCalls)} sub={<span className="flex items-center gap-0.5"><Wrench size={8} /> total</span>} infoTooltip="Total number of AI tool/function calls executed across all sessions" />
        </>}
        {tk && tk.input > 0 && <>
          <KpiCard label="tokens in" value={formatNumber(tk.input)} sub="prompt" infoTooltip="Total input tokens sent to AI models (prompt tokens)" />
          <KpiCard label="tokens out" value={formatNumber(tk.output)} sub={`${outputInputRatio}× ratio`} infoTooltip="Total output tokens generated by AI models (completion tokens)" />
          <KpiCard label="cache hit" value={`${cacheHitRate}%`} sub={formatNumber(tk.cacheRead)} infoTooltip="Percentage of prompt tokens served from cache (saves cost & latency)" />
          <KpiCard label="you wrote" value={formatNumber(tk.userChars)} sub={`AI: ${formatNumber(tk.assistantChars)}`} infoTooltip="Characters you typed vs characters AI generated" />
        </>}
      </div>
      ...
      {tk && tk.input > 0 && (
        <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))' }}>
          <KpiCard label="in tokens" value={formatNumber(tk.input)} sub="total prompt" infoTooltip="Total prompt tokens sent to AI models" />
          <KpiCard label="out tokens" value={formatNumber(tk.output)} sub="total completion" infoTooltip="Total completion tokens generated by AI models" />
          <KpiCard label="cache read" value={formatNumber(tk.cacheRead)} sub={`${cacheHitRate}% hit rate`} infoTooltip="Tokens read from prompt cache (saves cost)" />
          <KpiCard label="cache write" value={formatNumber(tk.cacheWrite)} infoTooltip="Tokens written to prompt cache for future reuse" />
          <KpiCard label="out/in ratio" value={`${outputInputRatio}×`} sub={<span className="flex items-center gap-0.5"><Zap size={8} /> efficiency</span>} infoTooltip="Output tokens per input token (higher = more verbose AI responses)" />
          <KpiCard label="you wrote" value={formatNumber(tk.userChars)} sub={`AI wrote ${formatNumber(tk.assistantChars)}`} infoTooltip="Your characters vs AI-generated characters" />
          <KpiCard label="est. cost" value={costs && costs.totalCost > 0 ? formatCost(costs.totalCost) : '—'} sub={costs && costs.byModel.length > 0 ? `${costs.byModel.length} model${costs.byModel.length !== 1 ? 's' : ''}` : undefined} infoTooltip="Estimated API cost based on token usage" />
        </div>
      )}
